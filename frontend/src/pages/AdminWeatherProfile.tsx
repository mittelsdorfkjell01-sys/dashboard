...
import { ApiError, getAdminWeatherDiagnostics, getAdminWeatherProfile, putAdminWeatherProfile, type WeatherProfileInput, type WeatherReferencePoint, type WeatherSector } from "../lib/api";
...
export default function AdminWeatherProfile() {
  ...
  return <div className="max-w-5xl"><UnsavedChangesDialog blocker={blocker}/><Link to="/admin/weather" className="text-admin-primary underline">← Wetterprofile</Link><h1 className="mt-3 text-2xl font-semibold">Wetterprofil</h1>
    ...
    <section className={sectionCls}><h2 className="text-lg font-semibold">E. Diagnostik</h2><button type="button" onClick={runDiagnostics} className="mt-3 rounded-md border border-admin-border px-3 py-2">Diagnostik laden</button>{diag && <div className="mt-3 space-y-2 text-ui"><p>Berechnung: {String((diag.internal as Record<string,unknown> | undefined)?.consensus_version ?? "—")}</p><p>Physik: {String((diag.internal as Record<string,unknown> | undefined)?.physics_version ?? "—")}</p><p>Qualitätsstufe: {String((diag.internal as Record<string,unknown> | undefined)?.quality_tier ?? "—")}</p><p>Forecasttage: {Array.isArray(diag.days) ? diag.days.length : 0}</p></div>}</section>
  </div>;
}
