...
import { activateAdminWeatherSectors, ApiError, getAdminForecastJob, getAdminWeatherDiagnostics, getAdminWeatherProfile, putAdminWeatherProfile, recalculateAdminForecast, type ForecastJob, type WeatherProfileInput, type WeatherReferencePoint, type WeatherSector } from "../lib/api";
...
export default function AdminWeatherProfile() {
  ...
  const [activationVersion, setActivationVersion] = useState(1);
  const [activationMinDrop, setActivationMinDrop] = useState("0.2");
  ...
  const load = useCallback(() => { setLoading(true); getAdminWeatherProfile(spotId).then((p) => {
    ...
    const candidates = p.sectors.filter((sector) => !sector.enabled).map((sector) => sector.version);
    const versions = p.sectors.map((sector) => sector.version);
    setActivationVersion(candidates.length ? Math.max(...candidates) : Math.max(0, ...versions) + 1);
  ...
  const activateSectors = async () => {
    setMessage(null); setErrors({});
    if (!activationRun.trim()) { setErrors({ activation: "Eine Gate-Run-ID ist erforderlich." }); return; }
    setActivating(true);
    try {
      const result = await activateAdminWeatherSectors(spotId, {
        version: activationVersion,
        runId: activationRun.trim(),
        minBiasDrop: Number(activationMinDrop),
        reason: activationReason.trim() || undefined,
      });
      setMessage(`Sektorversion ${result.activated_version} aktiviert (MAE-Verbesserung ${result.mae_drop.toFixed(3)} m/s).`);
      load();
    } catch (e) {
      setErrors({ activation: e instanceof ApiError ? e.message : "Aktivierung fehlgeschlagen." });
    } finally {
      setActivating(false);
    }
  };
  ...
  const publisherDiag = diag?.publisher as { shadow_profile?: { status: string; profile_class: string; algorithm_version: string; sectors: Array<{ index:number; center_deg:number; status:string; features:{ fetch?:{ first_land_median_m?:number|null; open_ray_fraction?:number|null; censored?:boolean }; terrain?:{ horizon_p90_deg?:number|null }; roughness?:{ index_median?:number|null; possible_double_counting?:boolean } }; quality:{ valid_rays?:number } }> } } | undefined;
  const shadow = publisherDiag?.shadow_profile;
  ...
  return <div className="max-w-5xl"><UnsavedChangesDialog blocker={blocker}/><Link to="/admin/weather" className="text-admin-primary underline">← Wetterprofile</Link><h1 className="mt-3 text-2xl font-semibold">Wetterprofil</h1>
    ...
    <section className={sectionCls}><h2 className="text-lg font-semibold">B. Basisdaten</h2><div className="mt-3 grid gap-4 sm:grid-cols-3">
      ...
      <label>Küstennormale (°)<input type="number" min={0} max={359.999} step="0.1" className={inputCls} value={form.coastal_normal_deg ?? ""} onChange={(e) => setForm({...form,coastal_normal_deg:n(e.target.value)})}/><span className="block text-caption text-admin-muted">Nord = 0°, im Uhrzeigersinn, vom Land zum Wasser. Fachliche Grundlage der Anströmungsklassifikation; niemals aus der Spot-/Strandausrichtung abgeleitet.</span></label>
    ...
    <section className={sectionCls}><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">D. Windsektoren</h2><button type="button" className="text-admin-primary underline underline-offset-4" onClick={() => setForm({...form,sectors:[...form.sectors,{start_deg:0,end_deg:45,speed_factor:1,direction_offset_deg:0,version:activationVersion,enabled:false,note:""}]})}>+ Kandidatensektor</button></div><p className="mt-2 text-caption text-admin-muted">Neue und bearbeitete Sektoren bleiben Kandidaten. Eine Aktivierung ist ausschließlich mit einem passenden WP1-Gate-Run möglich.</p>{sectorOverlap && <p role="alert" className="mt-2 text-admin-danger">Aktive Sektoren überschneiden sich.</p>}{errors.sectors && <p role="alert" className="mt-2 break-words text-admin-danger">{errors.sectors}</p>}
      <div className="mt-4 rounded-md border border-admin-border p-4"><h3 className="font-medium">WP1-Freigabe</h3><div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="min-w-0 text-caption">Gate-Run-ID<input maxLength={36} className={inputCls} value={activationRun} onChange={(e) => setActivationRun(e.target.value)} placeholder="UUID des Prüf-Runs"/></label><label className="text-caption">Kandidatenversion<input type="number" min={1} step={1} className={inputCls} value={activationVersion} onChange={(e) => setActivationVersion(Number(e.target.value))}/></label><label className="text-caption">Mindestverbesserung (m/s)<input type="number" min={0} step="0.01" className={inputCls} value={activationMinDrop} onChange={(e) => setActivationMinDrop(e.target.value)}/></label><label className="min-w-0 text-caption">Begründung<input maxLength={500} className={inputCls} value={activationReason} onChange={(e) => setActivationReason(e.target.value)} placeholder="Optional"/></label></div>{errors.activation && <p role="alert" className="mt-2 break-words text-admin-danger">{errors.activation}</p>}<button type="button" disabled={activating || !activationRun.trim() || !Number.isInteger(activationVersion) || activationVersion < 1 || !activationMinDrop.trim() || !Number.isFinite(Number(activationMinDrop)) || Number(activationMinDrop) < 0} onClick={activateSectors} className="mt-3 rounded-md bg-admin-primary px-4 py-2 font-medium text-admin-primary-fg disabled:opacity-50">{activating ? "Prüft und aktiviert …" : "Gate prüfen und Version aktivieren"}</button></div>
      {form.sectors.map((s,i) => <fieldset key={s.id ?? i} className="mt-3 grid min-w-0 gap-2 rounded-md border border-admin-border p-3 sm:grid-cols-7"><legend className="px-1">Sektor {i+1} · Version {s.version} · {s.enabled ? "aktiv" : "Kandidat"}</legend>{(["start_deg","end_deg","speed_factor","direction_offset_deg","version"] as const).map((key) => <label key={key} className="min-w-0 text-caption">{key}<input type="number" min={key === "version" ? 1 : undefined} step={key === "version" ? 1 : "any"} disabled={s.enabled} className={`${inputCls} disabled:cursor-not-allowed disabled:opacity-60`} value={s[key]} onChange={(e) => updateSector(i,{[key]:Number(e.target.value)})}/></label>)}<p className="self-end pb-2 text-caption font-medium">{s.enabled ? "Aktiv · schreibgeschützt" : "Nicht aktiv"}</p>{!s.enabled && <button type="button" className="self-end pb-2 text-admin-danger underline underline-offset-4" onClick={() => setForm({...form,sectors:form.sectors.filter((_,j)=>j!==i)})}>Löschen</button>}<label className="min-w-0 sm:col-span-7">Begründung/Quelle<input maxLength={500} disabled={s.enabled} className={`${inputCls} disabled:cursor-not-allowed disabled:opacity-60`} value={s.note ?? ""} onChange={(e) => updateSector(i,{note:e.target.value})}/></label></fieldset>)}
    </section>
    ...
    {shadow && <section className={sectionCls}><h2 className="text-lg font-semibold">F. Phase-2 Shadow-Sektoren</h2><p className="mt-2 text-caption text-admin-muted">{shadow.status} · Klasse {shadow.profile_class} · {shadow.algorithm_version}. Nur intern, ohne Forecast-Wirkung.</p><div className="mt-3 overflow-x-auto"><table className="min-w-full text-caption"><thead><tr className="text-left"><th>Richtung</th><th>Status</th><th>Fetch Median</th><th>Offen</th><th>Terrain P90</th><th>Rauigkeit</th><th>Strahlen</th></tr></thead><tbody>{shadow.sectors.map((s) => <tr key={s.index} className="border-t border-admin-border"><td>{s.center_deg}°</td><td>{s.status}</td><td>{s.features.fetch?.first_land_median_m == null ? (s.features.fetch?.censored ? "≥100 km" : "–") : `${Math.round(s.features.fetch.first_land_median_m/1000)} km`}</td><td>{s.features.fetch?.open_ray_fraction == null ? "–" : `${Math.round(s.features.fetch.open_ray_fraction*100)} %`}</td><td>{s.features.terrain?.horizon_p90_deg?.toFixed(1) ?? "–"}°</td><td>{s.features.roughness?.index_median?.toFixed(2) ?? "–"}{s.features.roughness?.possible_double_counting ? " ⚠" : ""}</td><td>{s.quality.valid_rays ?? 0}/9</td></tr>)}</tbody></table></div></section>}
  </div>;
}
