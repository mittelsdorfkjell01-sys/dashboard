...
def public_measurement(station, observation, *, now=None):
    ...
    if not isinstance(speed, (int, float)) or not 0 <= speed <= 75:
        reasons.append("wind_speed_invalid")
    if direction is not None and not 0 <= direction < 360:
        reasons.append("wind_direction_invalid")
    if gust is not None and gust < speed:
        reasons.append("gust_below_wind")
    quality = getattr(observation, "provider_quality", None)
    if quality not in ACCEPTED_QUALITY and str(quality).lower() not in ACCEPTED_QUALITY:
        reasons.append("provider_quality_rejected")
    if getattr(observation, "import_status", "accepted") != "accepted":
        reasons.append("import_rejected")
    if getattr(observation, "qc_version", None) and getattr(observation, "qc_flags", None):
