...
def _pilot_spots(
    ...
    *,
    ...
    if after_id:
        try:
            conditions.append(Spot.id > uuid.UUID(after_id))
        except (TypeError, ValueError):
            # A stale or manually damaged cursor must restart safely instead of
            # permanently starving every later catalog cycle.
            after_id = None
    return [
...
def _spot_page(db, *, bounds, limit: int, after_id: str | None) -> tuple[list, str | None]:
    """Return a deterministic page and its persisted continuation cursor."""
    rows = _pilot_spots(db, bounds=bounds, limit=limit, after_id=after_id)
    has_more = len(rows) > limit
    page = rows[:limit]
    return page, str(page[-1].id) if page and has_more else None
...
def _record_state(db, provider: str, *, error_class: str | None,
    ...
    # A transient provider failure must not erase the fair-pagination cursor;
    # otherwise every recovery restarts at the first European spot and the tail
    # can starve indefinitely. Successful cycles replace the progress snapshot.
    if error_class and not counts:
        state.last_counts = state.last_counts or {}
    else:
...
def _refresh_provider(
    ...
    *,
    ...
    elif provider == "awc_metar":
        items = awc_metar.fetch_stations(europe_only=True)
    else:
...
def run_station_catalog_refresh(
    ...
    *,
    ...
    page_size = spot_limit or settings.weather_station_catalog_spot_limit
    report = {"dry_run": dry_run, "providers": {}}
    now = datetime.now(timezone.utc)
    for provider in providers:
        ...
        if not force and state is not None and state.next_attempt_at is not None \
            ...
            continue
        try:
            previous_counts = state.last_counts if state and isinstance(state.last_counts, dict) else {}
            cursor = previous_counts.get("next_spot_cursor")
            spots, next_cursor = _spot_page(
                db, bounds=selected_bounds, limit=page_size, after_id=cursor
            )
            ...
            result["spot_page_size"] = len(spots)
            result["spot_cursor"] = cursor
            result["next_spot_cursor"] = next_cursor
            result["spot_cycle_complete"] = next_cursor is None
            ...
            if not dry_run and result["status"] == "success":
                _record_state(db, provider, error_class=None, counts={
                    ...
                    "spot_page_size": result["spot_page_size"],
                    "next_spot_cursor": result["next_spot_cursor"],
                    "spot_cycle_complete": result["spot_cycle_complete"],
                    **result["persistence"],
