...
| E2 | **Kitesurfen zuerst.** Die Personalisierung wird für `kitesurf` vollständig gebaut. Wing, Windsurf und Surf behalten bis auf Weiteres das bestehende Scoring, müssen aber architektonisch später andockbar sein (Sport als Parameter, keine Kite-Sonderwege in Kernschnittstellen). |
| E3 | **Nicht eingeloggte Nutzer** werden mit einem **Durchschnitts-Rider-Profil** bewertet (Abschnitt 4). Kein Session-Profil, keine Abfrage. |
