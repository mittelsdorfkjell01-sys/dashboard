...
|---|---|---|---|---|
| A1 | Vercel-„dashboard"-Projekt (Admin-Deploy) | Admin läuft als eigenes Vercel-Projekt (`dashboard`, `VITE_INCLUDE_ADMIN=true`, gleiches Repo). Unklar, ob dessen `DATABASE_URL` auf dieselbe befüllte Branch (`ep-polished-block`) zeigt wie Public → Risiko: Admin sieht andere/leere Daten. | Env des `dashboard`-Projekts verifizieren: `DATABASE_URL`(+`_UNPOOLED`) = Prod-Branch, `DB_SERVERLESS=true`, `REDIS_URL`, `MEDIA_BACKEND=blob`, `BLOB_READ_WRITE_TOKEN`, `JWT_SECRET`, `ADMIN_BOOTSTRAP_*`, `COOKIE_SECURE=true`, `ENABLE_ADMIN_API` **nicht** false. Ggf. angleichen + Redeploy. | P0 |
