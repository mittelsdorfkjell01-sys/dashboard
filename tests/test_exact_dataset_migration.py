        ...
        try:
            ...
            with target_engine.connect() as connection:
                assert connection.execute(text(
                    "SELECT version_num FROM alembic_version")).scalar_one() == "0062_station_capture_operations"
...
def test_disposable_0056_to_0059_keeps_legacy_residual_ineligible(db):
    ...
    try:
        ...
        _upgrade(url, "head")
        with target_engine.connect() as connection:
            assert connection.scalar(text("SELECT version_num FROM alembic_version")) == "0062_station_capture_operations"
            assert connection.scalar(text("SELECT count(*) FROM weather_observations")) == 1
